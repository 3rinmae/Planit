...
class TasksService {
  ...
  async updateTask(taskId, userId, taskData) {
    ...
    if (task.creatorId.toString() != userId) { throw new Forbidden('This is not your task therefore cannot update') }
  ...
  async destroyTask(taskId, userId) {
    const task = await this.getTaskById(taskId)
    if (task.creatorId.toString() != userId) { throw new Forbidden('This is not your task therefore cannot delete') }
    await task.remove()
